...
// suodatetaan konekohtaisesti eikä niitä rajata roolin mukaan (mekaanikot
// saavat hallita työmääräyksiä).
//
// writeRoles: jos annettu, POST/PUT/DELETE vaativat jonkin näistä rooleista
// (esim. ["admin"]) - GET-reitit ovat aina kaikkien kirjautuneiden käytössä.
export function crudFactory(collection, { writeRoles } = {}) {
  const router = Router();
  router.use(requireAuth);
  const requireWriteAccess = writeRoles ? [requireRole(...writeRoles)] : [];
  ...
  router.put("/:id", ...requireWriteAccess, asyncHandler(async (req, res) => {
